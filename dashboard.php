...
?>
      ...
      <div class="box">
      <?php
        $count_requests_sent_query ="SELECT * FROM `requests` WHERE sender = $user_id";
        $count_requests_sent_res=$conn->query($count_requests_sent_query);
     if ($count_requests_sent_res) {
       
   while ($total_requests_sent = $count_requests_sent_res->fetch_assoc()) {
   }
     }
      ?>
      <h3><?php $total_requests_sent; ?></h3>
      <p>requests sent</p>
      ...
      <div class="box">
      <?php
        $count_saved_properties_query ="SELECT * FROM `saved` WHERE user_id = $user_id";
        $count_saved_properties_res=$conn->query($count_saved_properties_query);
     
      if ($count_saved_properties_res) {
       
         while ($total_saved_properties = $count_saved_properties_res->fetch_assoc()) {
         }
           }
      ?>
      <h3><?php $total_saved_properties; ?></h3>
